...
- 套件 `app`（位於 `backend/app/`，以 hatchling editable 安裝，故 uvicorn/pytest/`python -m` 皆可 import）。
- `app.main`：FastAPI（`/healthz`、`/recommend`、`/chat`、`/line/webhook`）。
- `app.config`：keychain/env 憑證載入與 `Settings`。
- `app.conversation`：intent parser（port，預設 deterministic 關鍵字）、session store（in-memory）、`ConversationManager`（依缺失欄位追問，上限 3 題）。
- `app.line.webhook`：HMAC-SHA256 簽章驗證；`app.line.client`：reply API（httpx，port 可替換）。
- `app.importer`：`rule_to_rows` 純轉換 + `ServiceRepository` port + `import_rules`。
- `app.db`：psycopg 實作 repository（lazy import，僅設定 DATABASE_URL 時使用）。
- `app.rule_engine`：資格判斷（TASK.002）。
- `app.recommendation`：彙整可能服務的衝突檢查與合併文件 checklist（TASK.005），供 /recommend 與 /chat 共用。
- `app.reminders`：opt-in 提醒（`ReminderService` + `ReminderStore` port，in-memory；DB `reminder_tasks` 為後續）；`/reminders` CRUD 與 `/services`、`/services/{id}/source` 來源追溯端點（TASK.006）。
- `db/schema.sql`：PostgreSQL DDL。
- Degraded mode：無 DB/secret 時 API 仍可啟動，`/recommend` 由規則引擎服務。
