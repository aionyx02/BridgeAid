...
| `POST /chat` | `{session_id, message}` | `{kind: "question"\|"result"\|"info", text, options[], results[], conflicts[], document_checklist[]}` | 422 (驗證) |
...
| `GET /services/{id}/process` | — | `{service_id, service_name, steps[], version, needs_review, source}` | 404 |
